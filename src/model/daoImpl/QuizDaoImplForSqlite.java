...
public class QuizDaoImplForSqlite implements QuizDao {
	...
	//	@Override
	//	public Quiz generateAndInsertQuizToDb(int courseId,String courseName,int numOfQuestions) throws Exception {
	//		Quiz quiz=new Quiz();
	...
	//		quiz.setName("<html>"+courseName  +" Quiz"+"<br/"+"Started At "+dateTimeStr+"<html/>");
	//		quiz.setStartTime(currentDateTime);
	//
	//		fillQuizWithQuestionsAndTheirAnswersFromDb(quiz,courseName,QuizDao.NUM_OF_QUESTIONS);
	//
	//		int quizId=insertQuizWithItsQuestionsAndAnswersToDb(quiz);
	//		quiz.setID(quizId);
	//
	//		return quiz;
	//	}
	...
	public void fillQuizWithQuestionsAndTheirAnswers(Quiz quiz, String courseName, int numOfQuestionsInQuiz) throws Exception{
		ResultSet resultSet=null;
		try {
			...
			String sql="SELECT questions_pool.id AS question_id,questions_pool.text AS question_text,questions_pool.fk_right_answer_id,answers.id AS answer_id,answers.text AS answer_text " +
					...
					"ON questions_pool.fk_category_id=lessons_categories.id " +
					"INNER JOIN answers " +
					"ON answers.fk_question_id=questions_pool.id " +
					"WHERE lessons_categories.name=?";
	...
	public void updateQuizAsFinished(int quizId, Date timeFinished, int grade) throws Exception {
		try {
			...
			String sql="UPDATE generated_quizzes SET end_quiz_time=?, grade=? "+
					"WHERE id=?";
			statment = connection.prepareStatement(sql);
	...
	public boolean didStudentDoQuiz(int courseId, int studentId) throws Exception {
		...
		try {
			String sql="SELECT id " +
					...
					"WHERE fk_lesson_category_id=? AND fk_student_id=?";
	...
	public Quiz getQuiz(int courseId,int studentId) throws Exception{
		...
		try {
			String sql="SELECT generated_quizzes.id AS id," + 
					"generated_quizzes.name AS name," + 
					"generated_quizzes.start_quiz_time AS start_quiz_time," + 
					"generated_quizzes.end_quiz_time AS end_quiz_time," + 
					"generated_quizzes.grade AS grade," + 
	...
	public List<Question> getQuestionsOfQuiz(int quizId) throws Exception{
		...
		try {
			String sql="SELECT questions_pool.id AS id, " + 
					"questions_pool.text AS text, " + 
